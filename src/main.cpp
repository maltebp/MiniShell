...
void help()
{
    cout << "\nMiniShell is a small application implementing basic functionality:\n"
         ...
         << "  swap [FILE1] [FILE2] \t Swaps the content of the two given files.\n"
         ...
         << "  cat [FILE]           \t Prints the content of the given file.\n"
         << "  ping [HOST]          \t Pings the given host.\n"
         << "  env                  \t Prints the environment variables.\n"
         << "  setenv [VAR]=[VALUE] \t Sets the given environment variable to the given value.\n"
         << "  unsetenv [VAR]       \t Unsets the given environment variable.\n"
         << "  genenv               \t Generates environment variables from the given file.\n"
    ...
    while (1)
    {
        ...
        if (getInput(args))
        {
            ...
            else
            {
                ...
                {
                    system("clear"); // Execute the system command to clear the terminal screen
                }

                // Check if it is a ls command
                else if (args[0] == "ls")
                {
                    string path = ".";
                    if (args.size() > 1)
                ...
                else if (args[0] == "ping")
                {
                    ...
                    else
                    {
                        ...
                        executePing(hostname.c_str());
                    }
                }

                // Check if its a exportenv command
                else if (args[0] == "setenv")
                {
                    if (args.size() < 2)
                    {
                        cout << "Usage: setenv NAME=VALUE"
                             << endl;
                    }
                    else
                    {
                        for (int i = 1; i < args.size(); i++)
                            handleExport(args[i].c_str());
                    }
                }
                ...
                else if (args[0] == "unsetenv")
                {
                    if (args.size() < 2)
                    {
                        cout << "Usage: unsetenv NAME"
                             << endl;
                    }
                    else
                    {
                        for (int i = 1; i < args.size(); i++)
                            handleUnset(args[i].c_str());
                    }
                }

                // Check if its a env command
                else if (args[0] == "env")
                {
                    handleEnv();
                }

                // Check if its genenv command
                else if (args[0] == "genenv")
                {
                    string filename = ".env";
                    generateEnvFile(filename.c_str());
                }

                // Check if its a swap command
                else if (args[0] == "swap")
                {
                    if (args.size() != 3)
                    {
                        cout << "Usage: swap [FILE1] [FILE2]" << endl;
                    }
                    else
                    {
                        string file1 = args[1];
